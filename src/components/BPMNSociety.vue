    ...
    <div id="bpmn-app">
        <div class="container">
            ...
            <div class="action">
                ...
                <el-button-group style="margin-left: 10px;">
                    <el-button @click="restoreBPMN">保存修改</el-button>
                </el-button-group>
                <!-- 预览弹出 -->
...
import { restoreSocietyServicePattern } from '@/api/index'
...
`
    ...
    data() {
        return {
            ...
            scale: 1,
            canvas: null,
            bpmnTemplate: this.bpmnTemplateData,
            perviewXMLStr: null,
            perviewSVGData: null,
    ...
    methods: {
        async restoreBPMN() {
            ...
            var dataAll = { servicePatternName: name, data: xml,societyName:societyName }
            restoreSocietyServicePattern(dataAll).then((ret) => {
                this.$message({
                    type: "success",
                    message: "保存成功!"
                })
        ...
        async init() {
            ...
            var inputdata = this.$route.query.data
            console.log(inputdata)
            if (inputdata != "") {
                await this.createNewDiagram(inputdata);
            }
            // 监听画布点击事件，存储变化值
            var that = this
