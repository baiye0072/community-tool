            ...
            <div class="mindmap-tools" v-show="!readonly">
                ...
                <el-button :type="activedPanel === 'layout' ? 'primary' : 'default'" class="my-button" size="small"
                    ...
                    <div class="my-button">
                        ...
                        <div style="text-align: center; margin-top: 8%;margin-right: 20%;">保存修改</div>
                    </div>
                </el-button>
            </div>
...
import { reqMindMapInformation, restoreSocietyMindMap } from '@/api/index'
// 注册插件
...
export default {
    ...
    methods: {
        ...
        saveMindMap() {
            var societyName=this.$route.query.societyName
            if (societyName != "") {
                var dataAll = { societyName: societyName, data: mindMapInstance.getData().data, children: mindMapInstance.getData().children }
                restoreSocietyMindMap(dataAll).then((ret) => {
                    console.log("success")
        ...
        init() {
            reqMindMapInformation().then((ret) => {
                ...
                var tempData = this.$route.query.societyPatternData
